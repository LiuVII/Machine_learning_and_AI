...
# General
DATA_FILE_POS = "rt-polarity.pos"
DATA_FILE_NEG = "rt-polarity.neg"
NUM_CLASSES = 2
TEST_SIZE = 0.1
...
# Load data
x_text, y = data_helpers.load_data_and_labels(DATA_FILE_POS, DATA_FILE_NEG)
...
with tf.Graph().as_default():
	...
	with tf.name_scope("embedding"):
		Wem = tf.Variable(tf.random_uniform([vocab_size, EMBEDDING_SIZE], -1.0, 1.0), name="Wem")
	...
	def step(step_num, x_batch, y_batch, writer=None, train=True):
		...
		summaries, iloss, iaccuracy = sess.run(
			[train_summary_op, loss, accuracy], feed_dict=feed_dict)

		time_str = datetime.datetime.now().isoformat()
		print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step_num, iloss, iaccuracy))
		
		if train:
			train_summary_writer.add_summary(summaries, step_num)
		elif writer:
	...
	for batch in batches:
		...
		step(step_num, x_batch, y_batch)
		if step_num % EVALUATE == 0:
			print("\nEvaluation:")
			step(step_num, x_dev, y_dev, writer=dev_summary_writer, train=False)
			print("")
